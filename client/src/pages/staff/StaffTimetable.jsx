...
import { FaSearch, FaUserTie, FaClock, FaDoorOpen, FaCalendarAlt, FaBookOpen, FaPlus, FaEdit, FaTrash, FaArrowLeft, FaPrint, FaFileAlt } from 'react-icons/fa';
...
const StaffTimetable = () => {
                ...
                <div class="swal-custom-form">
                    ...
                    <div class="swal-field-group">
                         ...
                         <select id="day_of_week" class="swal2-input custom-select">
                             ${days.map(d => `<option value="${d}" ${entry?.day_of_week === d ? 'selected' : ''}>${d}</option>`).join('')}
                         </select>
                     </div>
                     <div class="swal-field-group">
    ...
    const handlePrint = async () => {
        ...
        const staffName = view === 'all'
            ? (staffList.find(s => s.emp_id === selectedStaff)?.name || selectedStaff || 'All Staff')
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .v-line { width: 1px; background: #fdba74; height: 100%; margin: 0 auto; min-height: 40px; }
    ...
    return (
        <Layout>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                ...
                {/* Staff selector: only shown in 'all' view AND not in view-only mode */}
                ...
                {view === 'my' ? (
                    ...
                    <TimetableGrid days={days} displaySlots={displaySlots} timetable={timetable} loading={loading} showStaffName={true} getPeriodConfig={getPeriodConfig} handleAction={handleAction} handleDelete={handleDelete} view={view} />
                )}
            </motion.div>
        </Layout>
    );
                    ...
                    <tbody>
                        {days.map((day, dayIdx) => (
                            <tr key={day} className="group">
                                <td className="p-3 border-b border-r border-sky-50 bg-gray-50/30 text-center font-black text-gray-700 text-[10px] lg:text-xs uppercase tracking-widest whitespace-nowrap w-32 md:w-40">{day}</td>
                                {displaySlots.map((slot, idx) => {
                                    const isBreak = slot.is_break;
                                    if (isBreak) {
                                    ...
                                    const entries = timetable.filter(t => t.day_of_week === day && t.period_number === p);
                                    return (
                                        <td key={idx} className="p-2 border-b border-r border-sky-50 align-top min-h-[80px] h-24 relative w-[140px] max-w-[140px]">
                                            <AnimatePresence>
                                                {entries.map(entry => (
                                                    <motion.div
                                                        key={entry.id}
                                                        ...
                                                        className="h-full min-w-0 bg-white border-2 border-sky-100 p-2.5 md:p-3 rounded-2xl shadow-lg shadow-sky-50/50 flex flex-col justify-between overflow-hidden relative group/entry hover:border-sky-400 transition-all"
                                                    >
                                                        ...
                                                        <div className="mt-2 pt-2 border-t border-gray-50 flex flex-col gap-1 min-w-0">
                                                            <div className="flex items-center gap-1.5 text-[8px] md:text-[9px] font-black text-gray-400 uppercase tracking-widest min-w-0">
                                                            ...
                                                            {entry.room_number && (
                                                                <div className="flex items-center gap-1 text-[8px] md:text-[9px] font-black text-sky-500 uppercase tracking-widest min-w-0">
                                                                    <FaDoorOpen size={9} /> <span className="truncate">{entry.room_number}</span>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </motion.div>
